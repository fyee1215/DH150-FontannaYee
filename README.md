...
**Evaluation (Rating: 2)**
During usage, there is distracting music playing in the background while meditation exercises or masterclasses are playing simultaneously, and it is extremely unclear where the music is coming from and how to pause it. To solve this problem, I would add the background music to the same status bar at the bottom of the page that appears when other audio is playing, so the user can easily pause the music. 
...
**Evaluation (Rating: 1)**
The names of some of the meditation exercises, such as “Take:90” are chosen for aesthetics more than practicality, and are ambiguous to the user because it is unclear what the exercise is going to be about. There are also no descriptions of any of the exercises once the user clicks on it. A simple solution would be to include a brief description of the purpose of each exercise before the user starts the meditation. 
...
**Evaluation (Rating 1)**
During the initial evaluation, there is no option to go back to the previous page in case the user selected an item they did not mean to select. This could potential cause the website to recommend inaccurate meditation recommendations that do not cater to the user’s needs. A simple solution would be to include a back option during the questionnaire. 

**4. Consistency and standards** 
 * Users should not have to wonder whether different words, situations, or actions mean the same thing. 
 * Follow platform conventions. 

**Evaluation (No Issues)**
The website demonstrates consistency very well, as each category on the menu bar is structured in the same way, with multiple boxes in a grid-like pattern. The user is quickly accustomed to the way the platform works, and each category label makes it clear what the purpose of that page is. 
...
**Evaluation (Rating: 1)**
The website did not warn users that their answers during the evaluation would be their final answers for the duration of their use of Calm. I would include a message at the end of the survey to ask users to confirm their answers before providing a final catered list of recommendations. 
...
**Evaluation (Rating: 2)** 
For the “Scenes” category, the user sees a variety of options for images and a sound bar at the top to increase or decrease volume, but it is extremely unclear what the purpose of this category is, and clicking on a random image doesn’t make it more clear. The volume bar also appears arbitrary in a category filled with visual images. One way to solve this issue is to show a preview of what a change of scene would look like for the site as a whole, and clearly label what the purpose of the volume bar is. 
...
**Evaluation (Rating: 2)**
The multiple different possible appearances of each of the categories, from status bar at the bottom of the page to full screen, cater to the different needs of experienced users but are very confusing to novice users of Calm. When an exercise or audio clip appears in full screen, the user loses access to the menu bar, which allows new users to get lost because the navigation system is hidden, and because it is ambiguous what category the clip they are watching belongs to. To solve this, I would make sure that even in “full screen” mode, the menu bar on the side is still shown. I would also include a caption of what category the video playing belongs to in order to help orient the user. 
...
**Evaluation (Rating: 1)**
As a whole, the website has a very clean, streamlined format that allows for easy use. However, the sheer number of boxes representing meditation options could overwhelm a new user. Instead of displaying a “show all” option for all the available clips, I would immediate divide the clips into categories, so the user sees fewer boxes and would be less overwhelmed. 
...
**Evaluation (Rating: 2)**
When an audio clip freezes and stops playing, this website offers no explanation for what the problem may be, and whether it is an issue with their audio clip or the user’s network. A solution would be to notify the user of what exactly the problem is when the audio malfunctions and help the user troubleshoot the problem, such as asking them to refresh the page. 
...
**Evaluation (Rating: 3)**
The support feature is only located at the very bottom of the profile page, and there are only preselected questions and concerns. If the user had a concern that wasn’t already presented, there is also no available contact information they can find to contact Calm. I would include an imbedded email box to send concerns to Calm and possibly also include a phone number or email address. 
